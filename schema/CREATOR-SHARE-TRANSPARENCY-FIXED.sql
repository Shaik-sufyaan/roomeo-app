-- ===============================
-- CREATOR SHARE TRANSPARENCY FIX (FIXED CONSTRAINT VERSION)
-- Creator shows their share but is marked as already paid
-- Fixes check constraint issue
-- ===============================
...
-- STEP 2: Drop the problematic constraint that prevents creators from paying more than they owe
ALTER TABLE expense_participants DROP CONSTRAINT IF EXISTS valid_payment_amount;

-- STEP 3: Add a new constraint that allows creators to pay more than they owe
ALTER TABLE expense_participants ADD CONSTRAINT valid_payment_amount_flexible 
CHECK (
  amount_paid <= amount_owed OR 
  (amount_paid > amount_owed AND is_settled = true)
);

-- STEP 4: Drop existing functions to recreate them
...
-- STEP 5: Create FIXED create_expense_group function (includes creator with transparency)
...
-- STEP 6: Create FIXED create_event_room function (includes creator with transparency)
...
-- STEP 7: Create FIXED create_regular_room function (includes creator with transparency)
...
-- STEP 9: Update existing expense rooms to include creator transparency
...
-- STEP 10: Recalculate amounts for existing participants after adding creator
...
SELECT 'Creator transparency fix applied with constraint fix!' as status;
