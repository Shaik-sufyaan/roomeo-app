-- ===============================
-- PRODUCTION SAFE Quick Access Feature Migration
-- ===============================
-- NO TEST DATA INSERTS - Safe for production use
...
-- ===============================
-- STEP 4: Update constraint safely (auto-detect real constraint name)
-- ===============================
...
BEGIN
    -- Find the actual userType constraint
...
-- ===============================
-- STEP 8: Final verification (NO TEST DATA INSERTS)
-- ===============================
...
BEGIN
    ...
    -- Check new constraint exists and has correct definition
    SELECT EXISTS (
    ...
    WHERE cc.constraint_name = 'check_user_type';
    ...
    IF new_constraint_check NOT ILIKE '%quick_access%' THEN
        RAISE EXCEPTION 'Migration failed: new constraint does not include quick_access';
    END IF;
    ...
    RAISE NOTICE '';
    RAISE NOTICE 'New constraint: %', new_constraint_check;
END $$;
...
-- ===============================
-- MANUAL VERIFICATION COMMANDS (run after migration)
-- ===============================

-- 1. Verify constraint allows quick_access (this should work now):
-- SELECT 'quick_access' IN (
--     SELECT unnest(string_to_array(
--         regexp_replace(check_clause, '.*ARRAY\[(.*?)\].*', '\1'),
--         ','
--     ))
--     FROM information_schema.check_constraints 
--     WHERE constraint_name = 'check_user_type'
-- ) as quick_access_allowed;

-- 2. View analytics:
-- SELECT * FROM user_type_analytics;

-- 3. Check upgrade function works (when you have quick_access users):
-- SELECT routine_name, routine_definition 
-- FROM information_schema.routines 
-- WHERE routine_name = 'track_user_upgrade';

-- 4. Verify all production data intact:
